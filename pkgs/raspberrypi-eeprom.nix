  ...
  fixupPhase = ''
    ...
    for i in rpi-eeprom-update rpi-eeprom-config; do
      wrapProgram $out/bin/$i \
        ...
        ${lib.optionalString stdenvNoCC.hostPlatform.isAarch64 "--set VCMAILBOX ${raspberrypi-utils}/bin/vcmailbox"} \
        --prefix PATH : "${
          lib.makeBinPath (
            [
              binutils-unwrapped
            ...
            ++ lib.optionals stdenvNoCC.hostPlatform.isAarch64 [raspberrypi-utils]
          )
        }"
    done
  '';
